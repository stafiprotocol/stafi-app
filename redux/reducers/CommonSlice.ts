...
export default class CommonSlice {
  async getPools(rsymbol: rSymbol, symbol: Symbol, cb?: Function) {
    const stafiApi = await stafiServer.createStafiApi();
    const poolsData = await stafiApi.query.rTokenLedger.bondedPools(rsymbol);
    ...
    if (pools && (pools as AnyJson[]).length > 0) {
      ...
      pools.forEach((poolPubKey: any) => {
        stafiApi.query.rTokenLedger
          .bondPipelines(rsymbol, poolPubKey)
          .then((bondedData: any) => {
            let active = 0;
  ...
  async poolBalanceLimit(rsymbol: rSymbol) {
    const stafiApi = await stafiServer.createStafiApi();
    const result = await stafiApi.query.rTokenSeries.poolBalanceLimit(rsymbol);
